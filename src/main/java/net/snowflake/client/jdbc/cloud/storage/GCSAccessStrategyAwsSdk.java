...
public class GCSAccessStrategyAwsSdk implements GCSAccessStrategy {
  ...
  GCSAccessStrategyAwsSdk(StageInfo stage) {
    ...
    String endpoint = stage.gcsCustomEndpoint().or(()-> Optional.of("storage.googleapis.com")).get();
  ...
  public StorageObjectMetadata getObjectMetadata(String remoteStorageLocation, String prefix) {
    ...
    Map<String, String> userMetadata =
        meta.getRawMetadata().entrySet().stream()
            ...
            .collect(Collectors.toMap(e -> e.getKey().replaceFirst("x-goog-meta-", ""), e -> e.getValue().toString()));
  ...
  public void uploadWithDownScopedToken(
      ...
      String queryId)
      throws InterruptedException {
    // we need to assemble an ObjectMetadata object here, as we are not using S3ObjectMatadata for GCS

...
public class AwsSdkGCPSigner extends AWS4Signer {
  private static final SFLogger logger =
      SFLoggerFactory.getLogger(net.snowflake.client.core.SFSession.class);
  private static final Map<String, String> awsgcpHeaderMap =
  ...
  public void sign(SignableRequest<?> request, AWSCredentials credentials) {
    ...
    if (request.getHttpMethod() == HttpMethodName.GET) {
      // todo: why deflate (copied from pyuthon client
    ...
    for (Map.Entry<String, String> entry : headerCopy.entrySet()) {
      ...
      if (entry.getKey().toLowerCase().startsWith("x-amz-meta-")) {
        request.addHeader(entry.getKey().replace("x-amz-meta-", "x-goog-meta-"), entry.getValue());
      }
    }
  }
}
